...
// e.g., ./testPlain task1 10000 (or 20000 or 50000)
// e.g., ./testPlain task2 0 500
// e.g., ./testPlain task3 0
...
int main(int argc, char *argv[])
{
    ...
    if(task == "task1"){
        long nvariant = atoi(argv[2]);                      // number of variants
        ...
        int encoding = atoi(argv[2]);           // genotype encoding method: 0 or 1
        int n_individuals = atoi(argv[3]);      // number of individuals: 500 or 1000
        int n_alleles_trials = 5;               // number of alleles: 1,2,3,4,5
        ...
        run_variant_aggregation(n_posns, encoding, n_individuals, n_alleles_trials);
    }
    else if(task == "task3"){
        int n_posns = 6601984;                   // number of positions
        int encoding = atoi(argv[2]);           // genotype encoding method: 0 or 1
        ...
        run_matrices_aggregation(n_posns, encoding, n_alleles_trials);
    }
